...
def classify_curator_response(
    ...
    if vault_allocs:
        ...
        # ── Meaningful exit: when allocation first dropped below 50% of peak ──
        half_peak = profile["peak_toxic_supply_usd"] * 0.5
        if half_peak > 1.0:
            half_exit_rows = after_peak[after_peak["supply_assets_usd"] <= half_peak]
            if len(half_exit_rows) > 0:
                first_half = half_exit_rows.sort_values("timestamp").iloc[0]
                profile["first_half_exit_ts"] = int(first_half["timestamp"])
                profile["first_half_exit_date"] = first_half["date"]
            else:
                profile["first_half_exit_ts"] = None
                profile["first_half_exit_date"] = None
        else:
            profile["first_half_exit_ts"] = None
            profile["first_half_exit_date"] = None
    ...
    # ── Classification ──
    # Use meaningful exit signals only:
    #   - first_half_exit_ts: allocation dropped below 50% of peak (from timeline)
    #   - first_zero_alloc_ts: allocation reached zero (full exit)
    #   - first_cap_zero_ts: supply cap explicitly set to 0 (admin action)
    # Removed: first_realloc_withdraw_ts (tiny withdrawals inflated early-exit counts)
    action_timestamps = []
    if profile.get("first_half_exit_ts"):
        action_timestamps.append(profile["first_half_exit_ts"])
...
def main():
    ...
    alloc_path_gz = out_dir / "block3_allocation_timeseries.csv.gz"
    admin_path = out_dir / "block3_admin_events.csv"

    if alloc_path_gz.exists():
        df_alloc = pd.read_csv(alloc_path_gz, compression="gzip")
        all_alloc_rows = df_alloc.to_dict("records")
        print(f"📂 Loaded {len(all_alloc_rows)} allocation rows from Part A (gzip)")
    elif alloc_path.exists():
    ...
    if all_realloc_rows:
        ...
        realloc_path = out_dir / "block3_reallocations.csv.gz"
        df_realloc.to_csv(realloc_path, index=False, compression="gzip")
    ...
    for curator in df_p["curator_name"].unique():
        ...
        for _, r in c_vaults.iterrows():
            ...
            if pd.notna(r.get("first_half_exit_date")):
                events.append(f"alloc<50%: {r['first_half_exit_date']}")
