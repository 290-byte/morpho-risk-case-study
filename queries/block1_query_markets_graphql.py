...
def main():
    ...
    # Also fetch historical supply per market
    fetch_market_historical_supply()


def fetch_market_historical_supply():
    """
    For each toxic market, query historicalState to get total supply
    at the depeg date (Nov 3-4, 2025). Requires block1_markets_graphql.csv.
    Outputs: block1_market_historical_supply.csv
    """
    print(f"\n{'=' * 80}")
    print("Block 1.2: Historical market supply (pre-depeg)")
    print("=" * 80)

    output_dir = PROJECT_ROOT / "data"
    markets_path = output_dir / "block1_markets_graphql.csv"
    if not markets_path.exists():
        print("ERROR: block1_markets_graphql.csv not found")
        return

    df = pd.read_csv(markets_path)
    print(f"Loaded {len(df)} markets")

    # Query window: Oct 1 - Nov 15, 2025 (daily)
    START_TS = 1759276800   # Oct 1 2025
    END_TS   = 1763164800   # Nov 15 2025
    DEPEG_TS = 1762128000   # Nov 3 2025 (last full day before crash)
    ...
    for _, row in df.iterrows():
          ...
          marketByUniqueKey(uniqueKey: "%s", chainId: %d) {
            ...
            historicalState {
              supplyAssetsUsd(options: {
                startTimestamp: %d, endTimestamp: %d, interval: DAY
              }) { x y }
        ...
        """ % (market_id, chain_id, START_TS, END_TS, START_TS, END_TS)

        try:
            data = query_graphql(query)
            market_data = data.get("data", {}).get("marketByUniqueKey", {})
            hist = market_data.get("historicalState", {})
            supply_ts = hist.get("supplyAssetsUsd", [])
            borrow_ts = hist.get("borrowAssetsUsd", [])
            ...
            for pt in supply_ts:
                ts, val = pt["x"], pt["y"]
                peak_supply = max(peak_supply, val or 0)
                if ts <= DEPEG_TS:
                    supply_at_depeg = val or 0

            for pt in borrow_ts:
                ts, val = pt["x"], pt["y"]
                if ts <= DEPEG_TS:
                    borrow_at_depeg = val or 0
            ...
            results.append({
                ...
                "supply_at_depeg": supply_at_depeg,
                "borrow_at_depeg": borrow_at_depeg,
                "peak_supply_oct_nov": peak_supply,
                "supply_now": float(row["total_supply_usd"]),
            })
            print(f"  {sym:8s}/{chain:10s} depeg=${supply_at_depeg:>12,.0f}  peak=${peak_supply:>12,.0f}  now=${float(row['total_supply_usd']):>12,.0f}")

        except Exception as e:
            print(f"  {sym:8s}/{chain:10s} ERROR: {e}")
            results.append({
                "market_id": market_id, "chain": chain, "chain_id": chain_id,
                "collateral_symbol": sym,
                "supply_at_depeg": 0, "borrow_at_depeg": 0,
                "peak_supply_oct_nov": 0, "supply_now": float(row["total_supply_usd"]),
            })

    out = pd.DataFrame(results)
    out_path = output_dir / "block1_market_historical_supply.csv"
    out.to_csv(out_path, index=False)

    total_depeg = out["supply_at_depeg"].sum()
    total_peak = out["peak_supply_oct_nov"].sum()
    total_now = out["supply_now"].sum()

    print(f"\n{'─' * 60}")
    print(f"  Total supply at depeg (Nov 3): ${total_depeg:>14,.0f}")
    print(f"  Peak supply (Oct-Nov):         ${total_peak:>14,.0f}")
    print(f"  Current supply:                ${total_now:>14,.0f}")
