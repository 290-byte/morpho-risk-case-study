...
def write_snapshot():
    ...
    def read(name):
        ...
        if p.exists():
            ...
            if "vault_name" in df.columns and "vault_address" in df.columns:
                dup = df["vault_name"].str.contains("Duplicated Key", case=False, na=False)
                for idx in df[dup].index:
                    addr = str(df.loc[idx, "vault_address"])
                    df.loc[idx, "vault_name"] = f"Vault {addr[:6]}...{addr[-4:]}"
            return df
        return pd.DataFrame()
