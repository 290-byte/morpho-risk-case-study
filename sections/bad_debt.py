...
from utils.data_loader import load_markets, load_vaults, load_share_prices, load_bad_debt_detail, load_market_supply_at_depeg
from utils.charts import fmt_usd_cols,  apply_layout, depeg_vline, RED, GREEN, BLUE, YELLOW, ORANGE, format_usd
...
def render():
    ...
    # ── Key Metrics ─────────────────────────────────────────
    # Public bad debt from block1
    total_bad_debt = markets["bad_debt_usd"].sum()
    markets_with_debt = len(markets[markets["bad_debt_usd"] > 0])

    # Private market loss from block8 (Elixir-Stream supply at depeg)
    mkt_depeg = load_market_supply_at_depeg()
    if not mkt_depeg.empty:
        es_rows = mkt_depeg[mkt_depeg["market_label"].str.contains("ES", na=False)]
        private_capital_lost = es_rows["supply_usd"].sum() if not es_rows.empty else 0
    else:
    ...
    st.caption(
        ...
        f"plus {format_usd(private_capital_lost)} lost in a private Elixir-Stream market on Plume. "
        f"Bad debt figures reflect current protocol state and accrue over time as interest compounds on unrepayable positions.".replace("$", "\\$")
    )
    ...
    c1.metric("Public Bad Debt", f"${total_bad_debt:,.0f}", delta=f"-${total_bad_debt:,.0f}", delta_color="inverse",
              help="Current protocol state. Accrues as interest compounds on positions borrowers will not repay.")
    ...
    # Private market context (from block8 Elixir-Stream data)
    if private_capital_lost > 0:
        st.warning(
            f"**Private market (Plume, not shown above).** "
            f"An unlisted xUSD/USDC market on Plume held roughly "
            f"**{md_usd(private_capital_lost)}** in USDC supply at the time of the depeg. "
            f"This was a direct Elixir Network deposit with Stream Finance as sole borrower. "
            f"Interest has since inflated the nominal position, but the relevant figure is the "
            f"original capital lost when the collateral became worthless."
        )
    ...
    with tab2:
        ...
        if not stable.empty:
            stable_display = stable[["vault_name", "curator", "tvl_usd", "share_price", "response_class"]].copy()
            stable_display["share_price"] = stable_display["share_price"].apply(lambda x: f"{x:.4f}")
            stable_display["tvl_usd"] = stable_display["tvl_usd"].apply(format_usd)
            st.dataframe(
                stable_display,
                column_config={
                    "vault_name": "Vault",
