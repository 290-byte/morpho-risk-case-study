...
from utils.charts import fmt_usd_cols,  apply_layout, donut_chart, RED, BLUE, ORANGE, GREEN, YELLOW, format_usd
...
def render():
    ...
    if not exposure_raw.empty:
        ...
        if display_cols:
            ...
            if "primary_chain" in df_show.columns:
                df_show["primary_chain"] = df_show["primary_chain"].str.title()
            # Rename "Duplicated Key" vaults to truncated address
            if "vault_name" in df_show.columns:
                dup = df_show["vault_name"].str.contains("Duplicated Key", case=False, na=False)
                if dup.any() and "vault_address" in exposure_raw.columns:
                    for idx in df_show[dup].index:
                        addr = str(exposure_raw.loc[idx, "vault_address"])
                        chain = str(df_show.loc[idx, "primary_chain"]) if "primary_chain" in df_show.columns else ""
                        df_show.loc[idx, "vault_name"] = f"Vault {addr[:6]}...{addr[-4:]}" + (f" ({chain})" if chain else "")
                elif dup.any():
                    df_show = df_show[~dup].copy()
            if "total_supply_usd" in df_show.columns:
                df_show = df_show.sort_values("total_supply_usd", ascending=False)
            st.dataframe(fmt_usd_cols(df_show), use_container_width=True, hide_index=True,
                         column_config={
                             "vault_name": "Vault",
                             "primary_chain": "Chain",
                             "n_toxic_markets": st.column_config.NumberColumn("Toxic Markets", format="%d"),
                             "toxic_markets": "Market Names",
                             "collateral_types": "Collateral",
                             "total_supply_usd": "Exposure (USD)",
