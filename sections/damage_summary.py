...
from utils.data_loader import (
    ...
    load_asset_prices, load_csv, load_market_supply_at_depeg,
)
from utils.charts import fmt_usd_cols,  apply_layout, depeg_vline, RED, BLUE, ORANGE, GREEN, YELLOW, format_usd
...
def render():
    ...
    # ══════════════════════════════════════════════════════════
    # Block8 provides verified hourly supply at depeg for each toxic market.
    # The ES (Elixir-Stream) market is private and goes into Cat 4 only.
    ES_MARKET_ID = "0x82e7ab8ccabaac59b5f397507ed031ebf19a9a5b2657c00c93bc2423cd0a890d"

    mkt_depeg = load_market_supply_at_depeg()
    b8_supply_lookup = {}
    if not mkt_depeg.empty:
        for _, row in mkt_depeg.iterrows():
            b8_supply_lookup[row["market_id"]] = float(row["supply_usd"])

    # Filter to 100%-util / bad-debt markets, exclude ES (private)
    full_util_markets = markets[
        markets["status"].str.contains("AT_RISK_100PCT|BAD_DEBT", na=False)
    ].copy()

    # Identify block1 column holding market hex ID
    _mkt_key_col = None
    for _candidate in ("market_id", "market_unique_key", "uniqueKey"):
        if _candidate in full_util_markets.columns:
            _mkt_key_col = _candidate
            break

    # Exclude Elixir-Stream from Cat 3 (it belongs in Cat 4)
    if _mkt_key_col:
        full_util_markets = full_util_markets[
            full_util_markets[_mkt_key_col] != ES_MARKET_ID
        ].copy()
    ...
    for _, m in full_util_markets.iterrows():
        ...
        # Use block8 depeg-time supply if available, else block1 current
        mkt_key = str(m.get(_mkt_key_col, "")) if _mkt_key_col else ""
        if mkt_key in b8_supply_lookup:
            supply_depeg = b8_supply_lookup[mkt_key]
        else:
            supply_depeg = supply_now

        locked_supply += supply_depeg
        locked_supply_now += supply_now
        ...
        true_loss = supply_depeg if is_toxic else 0
    ...
    # ══════════════════════════════════════════════════════════
    # CATEGORY 4: Private Market Exposure (block8 verified)
    # ══════════════════════════════════════════════════════════
    # ES market supply from block8 (verified hourly data)
    if ES_MARKET_ID in b8_supply_lookup:
        private_exposure = b8_supply_lookup[ES_MARKET_ID]
    else:
        # Fallback: check block1 supply_usd for ES market
        if _mkt_key_col and _mkt_key_col in markets.columns:
            es_rows = markets[markets[_mkt_key_col] == ES_MARKET_ID]
            private_exposure = float(es_rows["supply_usd"].sum()) if not es_rows.empty else 68_000_000
        else:
    ...
    if b8_supply_lookup:
        st.caption(
            "Supply at Depeg = verified hourly snapshot at November 3, 2025 00:00 UTC (depeg onset). "
    ...
    if locked_analysis:
        ...
        if active_locked:
            ...
            locked_df["chain"] = locked_df["chain"].str.title()
            ...
            col_config = {
                ...
                "chain": "Chain",
                "collateral": "Collateral",
                "supply_at_depeg": "Supply at Depeg",
                "supply_now": "Supply Now (inflated)",
                "bad_debt_recognized": "Bad Debt (Oracle)",
                "collateral_spot": "Collateral Spot Price",
                "true_loss": "True Loss (at depeg)",
                "oracle_gap": "Oracle-Masked Gap",
            }
            st.dataframe(
                fmt_usd_cols(locked_df[display_cols]),
            ...
            if oracle_masked_loss > 0:
                st.error(
                    f"**{md_usd(oracle_masked_loss)} in losses are not yet recognized by the protocol.** "
                    f"As of the last data pull, {md_usd(total_recognized_bd)} is flagged as bad debt. "
                    f"Both figures shift over time as interest accrues on unrepayable positions. "
                    f"Some borrowers (e.g. Re7 on pUSD) have repaid voluntarily; 5 liquidations recovered \\$241K."
                )
    ...
    damage_rows = [
        ...
        },
        {
            ...
            "Status": f"Confirmed on-chain: {format_usd(private_exposure)} USDC supply in unlisted Plume market at depeg (block8 verified). Interest has since inflated the nominal position. Single borrower (Stream Finance), direct supply by Elixir Network",
            "See Also": "Market Exposure",
        },
        {
            "Category": "5. Liquidity Contagion",
        ...
        },
        {
            ...
            "Scope": f"{n_markets_100} markets hit 100% util (+ 1 private)",
    ...
    st.dataframe(
        fmt_usd_cols(pd.DataFrame(damage_rows)),
        column_config={
