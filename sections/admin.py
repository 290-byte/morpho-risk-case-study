"""Section: Data & Methodology: pipeline overview, data freshness, field inventory, and pipeline runner."""
...
def _run_pipeline_streaming(block_names: list, log_widget) -> tuple:
    ...
    try:
        ...
        proc.kill()
        buffer.append("\n⏱ Pipeline timed out after 30 minutes.")
        full_output = "\n".join(buffer)
...
def render():
    ...
    st.info(
        "**Data freshness note.** Most metrics are pinned to historical timestamps "
        "(block8 hourly data at Nov 3 00:00 UTC) and do not change between runs. "
        "Two exceptions: **Bad Debt Recognized** and **Oracle-Masked Loss** come from "
        "the protocol's current state (block1). These accrue over time as interest "
        "compounds on positions borrowers will not repay. The sum converges toward "
        "the total supply at depeg (\\$15.1M) as the protocol catches up."
    )
    ...
    for title, block_id, description, files in blocks:
        with st.container(border=True):
            ...
            with col_info:
                ...
                for f in files:
                    fpath = DATA_DIR / f
                    fpath_gz = DATA_DIR / (f + ".gz") if not f.endswith(".gz") else fpath
                    if fpath.exists():
                        age = _file_age_str(fpath)
                        found.append(f"`{f}` ({age})")
                    elif fpath_gz.exists():
                        age = _file_age_str(fpath_gz)
                        found.append(f"`{f}.gz` ({age})")
                    else:
                        missing.append(f)
    ...
    # ── GraphQL Field Inventory ────────────────────────────────
    st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
    st.subheader("GraphQL Field Inventory")
    st.caption(
        "Every field queried from the Morpho GraphQL API across all pipeline blocks. "
        "Grouped by domain. This is the complete set of on-chain data points underlying the analysis."
    )

    field_data = {
        "Market State": [
            ("uniqueKey", "Market unique identifier (hash)"),
            ("lltv", "Liquidation loan-to-value threshold"),
            ("utilization", "Current utilization rate (borrows / supply)"),
            ("supplyAssets / supplyAssetsUsd", "Total supply in native + USD"),
            ("borrowAssets / borrowAssetsUsd", "Total borrows in native + USD"),
            ("supplyShares / borrowShares", "Supply and borrow share totals"),
            ("liquidityAssets / liquidityAssetsUsd", "Available liquidity"),
            ("collateralAssets / collateralAssetsUsd", "Collateral posted"),
            ("badDebt / badDebtAssets / badDebtUsd", "Current unrealized bad debt"),
            ("badDebtShare", "Bad debt share accounting"),
            ("realizedBadDebt", "Previously socialized bad debt"),
            ("fee", "Market fee rate"),
            ("irmAddress", "Interest rate model contract"),
            ("listed", "Whether market is publicly listed"),
            ("creationTimestamp", "Market deployment time"),
            ("supplyingVaults", "Vaults currently supplying to this market"),
            ("totalSupplyAssets", "Gross supply including protocol reserves"),
            ("supplyApy / borrowApy / netSupplyApy / netBorrowApy", "Current yield rates"),
        ],
        "Vault State": [
            ("totalAssets / totalAssetsUsd", "Vault TVL in native + USD"),
            ("totalSupply", "Vault share supply"),
            ("sharePriceNumber / sharePriceUsd", "Current share price"),
            ("allocation { market, supplyAssets, supplyAssetsUsd, supplyCap, supplyCapUsd }", "Per-market allocation breakdown"),
            ("supplyQueue / withdrawQueue", "Ordered market queues"),
            ("curator / curators { name, address, verified }", "Curator identity and verification"),
            ("fee / feeRecipient / accruedFee", "Vault fee configuration"),
            ("guardian / admin / timelock", "Governance and timelock"),
            ("pendingSupplyCap / pendingSupplyCapValidAt", "Pending cap changes (timelock)"),
            ("removableAt", "When a market can be removed from queue"),
            ("publicAllocatorConfig { flowCaps { maxIn, maxOut } }", "Public allocator limits"),
        ],
        "Oracle & Price": [
            ("oracle { address, data { ... on MorphoChainlinkOracleV2Data } }", "Full oracle configuration"),
            ("baseFeedOne / baseFeedTwo", "Chainlink base price feeds"),
            ("quoteFeedOne / quoteFeedTwo", "Chainlink quote price feeds"),
            ("baseOracleVault / quoteOracleVault", "Vault-based oracle components"),
            ("baseVaultConversionSample / quoteVaultConversionSample / vaultConversionSample", "Oracle conversion samples"),
            ("scaleFactor", "Oracle price scaling"),
            ("price / priceUsd", "Current asset prices"),
            ("historicalPriceUsd / historicalState", "Historical price timeseries"),
            ("collateralAtRisk / collateralPriceRatio", "Risk-at-price data"),
            ("healthFactor", "Borrower position health"),
        ],
        "Events & Transactions": [
            ("adminEvents { type, timestamp, hash, data }", "Vault admin actions (cap changes, queue reorders)"),
            ("vaultReallocates { timestamp, blockNumber, market, shares, assets, caller }", "Reallocation history"),
            ("publicAllocatorReallocates { timestamp, type, market, assets }", "PA rebalancing events"),
            ("transactions { type, timestamp, hash, assets, shares, user }", "Vault deposits/withdrawals"),
            ("MarketLiquidation { seizedAssets, repaidAssets, badDebtAssets, liquidator, borrower }", "Liquidation events"),
            ("marketPositions { user, borrowShares, collateral, healthFactor }", "Borrower positions"),
        ],
        "Infrastructure": [
            ("chainId / chain", "Network identifier"),
            ("collateralAsset / loanAsset { address, symbol, decimals, priceUsd }", "Token metadata"),
            ("morphoBlue { address, chain }", "Protocol contract reference"),
            ("assetByAddress { symbol, decimals, priceUsd }", "Token lookup"),
            ("network / factory / featured / verified", "Vault metadata flags"),
        ],
    }

    for category, fields in field_data.items():
        with st.expander(f"**{category}** ({len(fields)} groups)", expanded=False):
            for field_name, description in fields:
                st.markdown(f"`{field_name}`: {description}")
